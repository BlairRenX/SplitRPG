...
class Door(object):
    def __init__(self, doorID, room1, room2, direction, locked = None, blocked = None, opened=False, seeThrough = True, description = 'A basic, nondescript door. It looks quite old.', hidden = False):

        if locked is None:
            locked = [False,[],False]
            #[Is locked, [what unlocks/locks it], if it has indication of what locks it]
        if blocked is None or False:
            blocked = [False,None]
